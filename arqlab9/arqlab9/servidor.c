...
void process_request(int novo_soquete, FILE *log){
    ...
    while (1) {
        ...
        do{
            ...
            socket_value = select(1, &fdset, NULL, NULL, &tv);
...
{ 
    ...
    while (1) {
        ...
        tv.tv_sec = 2;             /* 2 segundos timeout */
        tv.tv_usec = 0;
        
        do{

            int socket_value = select(soquete + 1, &fdset, NULL, NULL, &tv);

        }while(errno==EINTR);

        if(FD_ISSET(soquete, &fdset)){
